...
local function logregen()
    ...
    mana:log(FMT_REGEN:format(regen/ (last - mana.lastregenlog)))
...
local function sample(self, elapsed)
    ...
    if mana.FSR then 
        ...
        if mana.FSR <= 0 then
            mana:log(("Non-Casting Regen %0.2f mp/5"):format(mana.db.regenbase*5))
            mana.FSR = nil
        end
    end
...
--		ChatFrame1:AddMessage(message, info.r, info.g, info.b);
    ...
    if sourceGUID == self.guid then
        ...
        if effectEvents[event] then
            if spellId == 16870 then
                self:log( ((event == "SPELL_AURA_APPLIED") and "+" or "-").."Clearcast")                
            end
            return
        end
        ...
        if powerType == 0 and (event == "SPELL_CAST_SUCCESS" or event == "SPELL_HEAL") then
            local regenloss = (powerCost > 0) and (5-(self.FSR or 0))*(self.db.regenbase-self.db.regencasting) or 0
            self:log(FMT_SPELL:format(name, powerCost, regenloss) .. ("  (%s)"):format(event))
            
            if not self.FSR and powerCost > 0 then
                self:log(("Casting Regen %0.2f mp/5"):format(self.db.regencasting*5))
            end
            if powerCost > 0 then
                self.FSR = 5
            end
